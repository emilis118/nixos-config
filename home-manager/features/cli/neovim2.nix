{
  ...
  # nixvim rebuild of the neovim.nix setup, exposed as `nvim2` so it can be
  # tested next to the main nvim before switching over.
  nvim2 = inputs.nixvim.legacyPackages.${pkgs.stdenv.hostPlatform.system}.makeNixvimWithModule {
    inherit pkgs;
    module = {
      # lua/emilis/set.lua
      opts = {
...
in {
  home.packages = [
    (pkgs.runCommand "nvim2" {} ''
      mkdir -p $out/bin
      ln -s ${nvim2}/bin/nvim $out/bin/nvim2
    '')
  ];
}
